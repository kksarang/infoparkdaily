...
export function field(
  ...
  return `<div class="field"><label for="${name}">${label}${required ? ' <span class="green">*</span>' : ""}</label><input id="${name}" name="${name}" type="${type}" value="${esc(value)}" ${required ? "required" : ""} maxlength="${max}" placeholder="${esc(placeholder)}" ${extra}>${help ? `<small>${help}</small>` : ""}</div>`;
}
export const area = (
  label,
  name,
  ...
  `<div class="field"><label for="${name}">${label}${required ? ' <span class="green">*</span>' : ""}</label><textarea id="${name}" name="${name}" maxlength="${max}" ${required ? "required" : ""} placeholder="${esc(placeholder)}">${esc(value)}</textarea>${help ? `<small>${help}</small>` : ""}</div>`;
