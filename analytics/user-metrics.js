/**
 * InfoparkDaily Analytics — User / session metrics (Phase 7)
 * ==========================================================
 * Most audience metrics come from GA4 automatically once GTM is live.
 * This module documents each KPI and collects client-side context GA4
 * cannot always see (network speed, exact screen, coarse device hints).
 */
(function (global) {
  "use strict";
  ...
  function collectClientContext() {
    ...
    var width = global.innerWidth || scr.width || 0;
    var height = global.innerHeight || scr.height || 0;
    ...
    var ctx = {
      ...
      network_rtt: "",
      save_data: false,
      touch: "ontouchstart" in global || (nav.maxTouchPoints || 0) > 0
    };
    ...
    // userAgentData when available (Chromium)
    try {
  ...
  function networkSpeedLabel(ctx) {
    ctx = ctx || {};
  ...
  global.IPD_ANALYTICS_USER_METRICS = {
    METRICS: METRICS,
    collectClientContext: collectClientContext,
    networkSpeedLabel: networkSpeedLabel,
    parseBrowser: parseBrowser,
    parseOS: parseOS,
    deviceCategory: deviceCategory,
    ga4BuiltinIds: METRICS.filter(function (m) {
      return m.source === "ga4" || m.source === "both";
    }).map(function (m) {
      return m.id;
    }),
    clientOnlyIds: METRICS.filter(function (m) {
