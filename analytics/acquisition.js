/**
 * InfoparkDaily Analytics — Acquisition / traffic sources (Phase 8)
 * ==================================================================
 * Classifies session channel from UTMs + document.referrer.
 * GA4 still owns default channel grouping; this adds InfoparkDaily-specific
 * labels (Instagram, WhatsApp, …) and locks first-touch UTMs for the session.
 */
(function (global) {
  "use strict";
  ...
  var PAID_MEDIA = ["cpc", "ppc", "paid", "paid_social", "display", "cpm", "cpv", "retargeting", "ads"];
  ...
  function classify(input) {
    ...
    // 2) Explicit UTM source → named channel
    if (!channel) {
    ...
    // 4) Any campaign id without clear channel → campaign bucket
    ...
    if (!channel && refHost) {
      if (selfHost && refHost === selfHost) {
        channel = null; // same-site; treat as continue / ignore
  ...
  function buildShareUrl(baseUrl, channel, vars) {
    vars = vars || {};
  ...
  global.IPD_ANALYTICS_ACQUISITION = {
    CHANNELS: CHANNELS,
    ...
    sourceToChannel: sourceToChannel,
    hostFromReferrer: hostFromReferrer
  };
