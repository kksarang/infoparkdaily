/**
 * InfoparkDaily Analytics — SEO metrics (Phase 16 ESM)
 * Documents GSC KPIs and runs a lightweight on-page audit.
 */
...
export const EXPECTED_HOST = "infoparkdaily.online";
...
export function auditPage(options) {
  options = options || {};
  var doc = options.document || (globalThis.document || null);
  var loc = options.location || globalThis.location || {};
  var issues = [];
  var summary = {
 ...
 */
export function probeInternalLinks(doc, loc, limit) {
  limit = limit || 8;
  doc = doc || globalThis.document;
  loc = loc || globalThis.location;
  if (!doc || !loc || typeof globalThis.fetch !== "function") {
...
export const gscMetricIds = METRICS.filter(function (m) {
  return m.source === "gsc" || m.source === "both";
}).map(function (m) {
  return m.id;
});

export const clientMetricIds = METRICS.filter(function (m) {
