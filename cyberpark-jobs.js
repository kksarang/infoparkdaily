(function () {
  const verifiedGridEl = document.getElementById("cp-verified-grid");
  const upcomingGridEl = document.getElementById("cp-upcoming-grid");
  const countEl = document.getElementById("cp-count");
  if (!verifiedGridEl || !upcomingGridEl || typeof CYBERPARK_VERIFIED_JOBS === "undefined") return;

  const ALL = CYBERPARK_VERIFIED_JOBS.slice();
  const verified = ALL.filter((job) => job.alertBucket === "verified");
  const upcoming = ALL.filter((job) => job.alertBucket !== "verified");
  const JOBS = ALL;
    ...
    return `
      <article class="job-card${expired ? " job-card--expired" : ""}${
        ...
        <div class="job-tag-row">
          <span class="job-tag-pill">CYBERPARK</span>
          ${(job.tags || [])
            .slice(0, 2)
        ...
        <footer class="job-card-foot">
          <div class="job-meta-row"><span class="job-source">Cyberpark</span></div>
  ...
  verifiedGridEl.innerHTML =
    verified.map(renderCard).join("") ||
    `<p class="jobs-empty">No verified Cyberpark listings in this set.</p>`;
  upcomingGridEl.innerHTML =
    upcoming.map(renderCard).join("") ||
    `<p class="jobs-empty">No upcoming Cyberpark alerts right now.</p>`;
