(function () {
  const verifiedGridEl = document.getElementById("ip-verified-grid");
  const upcomingGridEl = document.getElementById("ip-upcoming-grid");
  const countEl = document.getElementById("ip-count");
  if (!verifiedGridEl || !upcomingGridEl || typeof INFOPARK_VERIFIED_JOBS === "undefined") return;

  const ALL = INFOPARK_VERIFIED_JOBS.slice();
    ...
    return `
      <article class="job-card${expired ? " job-card--expired" : ""}${
        ...
        <div class="job-tag-row">
          <span class="job-tag-pill">INFOPARK</span>
          ${(job.tags || [])
            .slice(0, 2)
        ...
        <footer class="job-card-foot">
          <div class="job-meta-row"><span class="job-source">Infopark</span></div>
  ...
  verifiedGridEl.innerHTML =
    ...
    `<p class="jobs-empty">No verified listings in this set.</p>`;
  upcomingGridEl.innerHTML =
    upcoming.map(renderCard).join("") ||
    `<p class="jobs-empty">No upcoming Infopark alerts right now.</p>`;
